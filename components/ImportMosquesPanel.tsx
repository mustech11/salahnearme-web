...
export default function ImportMosquesPanel({ cities }: Props) {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/import-mosques?${params.toString()}`, {
        method: "GET",
      ...
      if (!response.ok || data.ok === false) {
        setSubmitState("error");
        setResult(cleanText(data.error) || "Mosque import failed.");
        return;
      }
      ...
      setResult(
        `Import complete for ${cleanText(data.city) || selectedCityRecord.name}. Found: ${data.found ?? 0}, inserted: ${data.inserted ?? 0}, skipped: ${data.skipped ?? 0}, invalid: ${data.invalid ?? 0}.`
      );
    } catch (error) {
      setSubmitState("error");
      setResult(
        error instanceof DOMException && error.name === "AbortError"
          ? timedOut
            ? "The mosque import timed out. Try a smaller radius."
            : "The mosque import was cancelled."
          : error instanceof Error
            ? error.message
            : "Unexpected mosque import error."
      );
    } finally {
  ...
  }, [radius, selectedCityRecord]);

  return (
    <section
      aria-labelledby="import-mosques-heading"
    ...
    >
      <div>
        ...
        <h2
          id="import-mosques-heading"
          className="mt-2 text-2xl font-black text-white md:text-3xl"
        >
          Import mosques by city
        </h2>

        <p className="mt-2 max-w-3xl text-sm leading-6 text-white/60">
          Run the OpenStreetMap mosque importer for one city with a controlled search radius.
        </p>
      </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1fr)_220px_auto]">
        ...
        <div className="flex items-end">
          ...
          >
            {loading ? "Importing…" : "Import mosques"}
          </button>
        </div>
      </div>

      <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-4 text-sm leading-6 text-white/55">
        Wider radiuses may include neighbouring areas. Start with 15,000 metres and reduce the radius if too many unrelated records are returned.
      </div>

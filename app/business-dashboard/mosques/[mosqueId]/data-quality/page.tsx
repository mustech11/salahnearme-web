...
function buildIssues({
  ...
  if (missingIqamah === 0) {
    ...
    issues.push({
      ...
      description:
        "Iqamah gaps reduce Pray Near Me accuracy and the “Can I make it?” feature.",
...
export default async function MosqueDataQualityPage({ params }: PageProps) {
  ...
  if (mosqueError) {
    return (
      <ErrorPanel message={mosqueError.message} />
    );
  }
  ...
  const { data: prayerRowsRaw, error: prayerRowsError } = await supabaseAdmin
  ...
  if (prayerRowsError) {
    return <ErrorPanel message={prayerRowsError.message} />;
  }

  const { data: jumuahRowsRaw, error: jumuahRowsError } = await supabaseAdmin
    .from("mosque_jumuah_times")
    .select("*")
    .eq("mosque_id", mosque.id);

  if (jumuahRowsError) {
    return <ErrorPanel message={jumuahRowsError.message} />;
  }
    ...
    <main className="mx-auto max-w-7xl px-4 py-10">
        ...
        <div className="grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
          <div className="rounded-3xl border border-white/10 bg-black/30 p-6">
            ...
            <p className="mt-4 text-sm leading-7 text-white/60">
              This score estimates how complete and reliable the mosque’s
              timetable data is for Pray Near Me, public timetable pages, and
              user-facing prayer guidance.
            </p>
          </div>
      ...
      <section className="mt-8 rounded-3xl border border-white/10 bg-black/20 p-6">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          {issues.map((issue) => (
            <div
              key={issue.label}
              className={`rounded-2xl border p-4 ${issueClass(issue.severity)}`}
            >
      ...
      <section className="mt-8 rounded-3xl border border-white/10 bg-black/20 p-6">
        ...
        <div className="mt-6 overflow-hidden rounded-2xl border border-white/10">
          <div className="grid grid-cols-[1.2fr_0.8fr_0.8fr_1fr] bg-white/5 px-4 py-3 text-xs font-bold uppercase tracking-[0.18em] text-white/40">
            <div>Date</div>
            <div>Begins</div>
            <div>Iqamah</div>
            <div>Status</div>
            ...
            {dateRange.map((date) => {
              ...
              if (!row) {
                return (
                  <div
                    key={date}
                    className="grid grid-cols-[1.2fr_0.8fr_0.8fr_1fr] px-4 py-3 text-sm text-white/70"
                  >
              ...
              return (
                ...
                >
                  ...
                  >
                    {formatLabel(row.confidence ?? row.source)}
                  </div>
                </div>
              );
            })}
